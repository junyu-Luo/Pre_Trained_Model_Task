...
class BertProcessor(object):
    ...
    def get_labels(self, config, split_symbol='\t',multi_symbol=','):
        """See base class."""
        if os.path.exists(config['label_list'] / 'label_list_bert.json'):
            label_list = read_json('label_list_bert.json', path=config['label_list'])
            return label_list
        else:
            labels = []
            ...
            save_json('label_list_bert.json', label_list, path=config['label_list'])
            return label_list
    ...
    #             for line in reader:
    #                 if len(line) > 0:
    #                     for i in line[0].split(split_symbol)[0].split(multi_symbol):
    #                         labels.append(i)
    #         label_list = sorted(list(set(labels)))
    #         if os.path.exists(config['label_list'] / 'label_list_bert.json') is False:
    #             save_json('label_list_bert.json', label_list, path=config['label_list'])
    #         return label_list
    ...
    def create_examples(self,lines,config,example_type,cached_examples_file=None,multi_symbol=','):
        '''
