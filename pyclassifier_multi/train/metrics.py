...
class MultiClassReport(Metric):
    ...
    def value(self):
        ...
        score = multi_train_metrics(real = self.y_true,
                                      pred = self.y_pred,
    ...
    def __call__(self,logits,target):
        ...
        # print('y_pred',y_pred)
        # print('y_true',y_true)
        pred_index = np.array([[1 if y else 0 for y in y_pred_] for y_pred_ in y_pred > 0.5])

        self.y_pred = [[[self.label_list[int(z)] for z in y] for y in np.where(x == 1)][0] for x in pred_index]
        self.y_true = [[[self.label_list[int(z)] for z in y] for y in np.where(x == 1)][0] for x in y_true]
