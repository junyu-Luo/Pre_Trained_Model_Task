...
from pyclassifier_multi.processors.xlnet_processor import XlnetProcessor
from pyclassifier_multi.processors.xlnet_processor import collate_fn #xlnet_collate_fn as collate_fn
from common.tools import init_logger, logger
from common.tools import seed_everything
from pyclassifier_multi.configs.config_multi_classifier import config
from pyclassifier_multi.model.xlnet_for_multi_label import XlnetForMultiLable
...
from pyclassifier_multi.train.metrics import AUC, AccuracyThresh, MultiClassReport
from pyclassifier_multi.callback.optimizater.adamw import AdamW
# from pyclassifier_multi.callback.optimizater.sgdw import SGDW
...
def run_train(args):
    ...
    # optimizer = SGDW(optimizer_grouped_parameters,lr=0.1)
...
def run_test(args):
    ...
    processor = XlnetProcessor(vocab_path=str(config['xlnet_vocab_path']), do_lower_case=args.do_lower_case)
    label_list = processor.get_labels(config) # data_dir=config['data_dir']
    id2label = {i: label for i, label in enumerate(label_list)}
    ...
    print()
    print('pred', [[[label_list[int(z)] for z in y] for y in np.where(x == 1)][0] for x in pred_index])
    print('real', [[[label_list[int(z)] for z in y] for y in np.where(x.label == 1.0)][0] for x in test_examples])
...
def main():
    ...
    init_logger(
        log_file=os.path.join(config['log_dir'], '{}.log'.format(str(time.strftime("%m%d_%H%M", time.localtime())))))
