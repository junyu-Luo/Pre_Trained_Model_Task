...
from pyclassifier_multi.processors.bert_processor import BertProcessor, collate_fn
from common.tools import init_logger, logger
from common.tools import seed_everything
from common.configs import get_config
from pyclassifier_multi.model.bert_for_multi_label import BertForMultiLable
...
def run_train(args):
    ...
    trainer = Trainer(args=args, model=model, logger=logger, criterion=BCEWithLogLoss(), optimizer=optimizer,
                      ...
                      epoch_metrics=[AUC(average='micro', task_type='binary'),
                                     MultiClassReport(label_list=label_list)])  # ,run_test=run_test(args)
...
def run_test(args):
    ...
    test_examples = processor.create_examples(lines=test_data,
                                              example_type='test', config=config,
                                              cached_examples_file=config[
    ...
    # if len(label_list)==2:
    #     print([label_list[np.argmax(x)] for x in result])
    #     print([[label_list[int(y)] for y in np.where(x.label == 1)][0] for x in test_examples])
    #
    # else:
    #     # print([[label_list[int(y)] for y in np.where(x.label == 1)] for x in test_examples])
    #     # print([[y for y in x.label] for x in test_examples])
    # print([[[label_list[z] for z in y] for y in np.where(x.label == 1.0)][0] for x in test_examples])
