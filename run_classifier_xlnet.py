...
from pyclassifier.processors.xlnet_processor import XlnetProcessor
from pyclassifier.processors.xlnet_processor import collate_fn  # xlnet_collate_fn as collate_fn
...
# from pyclassifier.callback.optimizater.sgdw import SGDW
...
def run_train(args):
    ...
    if args.resume_path:
        ...
        model = XlNetForSequenceClassification.from_pretrained(args.resume_path, num_labels=len(label_list))
    else:
        model = XlNetForSequenceClassification.from_pretrained(config['xlnet_model_dir'], num_labels=len(label_list))
    ...
    trainer = Trainer(args=args, model=model, logger=logger, criterion=BCEWithLogLoss(), optimizer=optimizer,
                      ...
                      epoch_metrics=[AccuracyThresh(thresh=0.5)], )
...
def run_test(args):
    ...
    processor = XlnetProcessor(vocab_path=str(config['xlnet_vocab_path']), do_lower_case=args.do_lower_case)
    # config['label_list'] / 'label_list_xlnet.json'
    label_list = processor.get_labels(config)
    id2label = {i: label for i, label in enumerate(label_list)}
...
def main():
    ...
    parser.add_argument("--resume_path", default='', type=str)  # ./output/classifier_task/checkpoints/xlnet
