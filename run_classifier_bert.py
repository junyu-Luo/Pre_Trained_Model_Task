...
from pyclassifier.processors.bert_processor import BertProcessor, collate_fn
from common.tools import init_logger, logger
from common.tools import seed_everything
# from pyclassifier.configs.config_multi_classifier import config
...
def run_train(args):
    # --------- data
    processor = BertProcessor(vocab_path=config['roberta_vocab_path'], do_lower_case=args.do_lower_case)
    ...
    train_examples = processor.create_examples(lines=train_data, config=config,
                                               example_type='train',
                                               cached_examples_file=config['cache_dir'] / f"cached_train_examples_{args.arch}")
    ...
    if args.resume_path:
        ...
        model = BertForSequenceClassification.from_pretrained(args.resume_path, num_labels=len(label_list))
    else:
        model = BertForSequenceClassification.from_pretrained(config['roberta_model_dir'], num_labels=len(label_list))
